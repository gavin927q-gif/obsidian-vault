---
type: project
status: pitching
client: Herring Properties
business_name: Herring Properties
contact_email: info@herringproperties.com  ⚠ BOUNCED — address does not exist
contact_phone: (254) 582-0022
contact_address: 101 N Waco St, Hillsboro, TX 76645
tier: Business Pro
price: 2500-4000
paid_deposit: false
paid_final: false
due:
domain: herringproperties.com
netlify_url:
business: Web Design
created: 2026-04-06
last_contact: 2026-04-07
lead_status: warm
next_action: CALL (254) 582-0022 or walk in — email bounced permanently
---

# Herring Properties — Website Rebuild

Local Hillsboro real estate firm. Currently on Wix — zero differentiation. Easy walk-in pitch.

## Pitch Status
- [x] Pitch email sent
- [ ] Follow-up #1
- [ ] Follow-up #2
- [ ] Proposal/PDF delivered
- [ ] Contract signed
- [ ] Deposit received
- [ ] Site in progress
- [ ] Site launched
- [ ] Final payment collected

## Notes
- Built on Wix — slow, poor SEO, generic template
- No agent profiles, no team photos, no local identity
- 20+ years in Hillsboro — not reflected on site
- Chamber of Commerce member
- Hours: 9 AM–5 PM — easy walk-in target
- **Best approach:** Walk in off the street, show their Wix site vs hillsborotireservice.com

## Communication Log
| Date | Method | Notes |
|------|--------|-------|
| 2026-03-30 | Email | Initial property services outreach (Hillsboro H&P) — no reply |
| 2026-04-07 | Email | Pitch email sent from hillsborowebdesign@outlook.com — BOUNCED. info@herringproperties.com does not exist (554 RecipientNotFound). |

## Next Action
- [ ] Call (254) 582-0022 — email is dead, must call or walk in
- [ ] Walk in: 101 N Waco St, Hillsboro — hours 9 AM–5 PM
